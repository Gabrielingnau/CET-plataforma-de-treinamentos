...
export function QuestionForm({ ownerId, type, defaultValues, onCancel, onSuccess }: any) {
  const isEditing = !!defaultValues;

  const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<QuestionFormData>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-accent/20 p-6 rounded-xl border border-accent">
      ...
      <div className="space-y-4">
        ...
        {errors.opcao_correta && <p className="text-xs text-destructive">{errors.opcao_correta.message}</p>}
      </div>

      <div className="flex justify-end gap-3 pt-4 border-t">
        <Button type="button" variant="ghost" onClick={onCancel}>Cancelar</Button>
        <Button type="submit" disabled={isSubmitting} className="bg-orange-600 hover:bg-orange-700">
          {isSubmitting ? "Salvando..." : isEditing ? "Atualizar Pergunta" : "Salvar Pergunta"}
        </Button>
      </div>
    </form>
  );
