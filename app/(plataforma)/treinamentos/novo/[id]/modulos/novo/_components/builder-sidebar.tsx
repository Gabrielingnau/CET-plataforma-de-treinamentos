...
export function BuilderSidebar({ structure, activeEditor, setEditor, trainingId }: any) {
  ...
  return (
    // h-full garante que a sidebar ocupe toda a altura do pai (que deve ser h-screen ou h-calc)
    <aside className="w-full lg:w-80 border-r bg-background flex flex-col shrink-0 overflow-hidden max-h-[70vh]">
      ...
      {/* Área de Conteúdo com Scroll Automático - Ocupa o espaço restante */}
            ...
            <div key={module.id} className="space-y-1">
              ...
              <div className="ml-4 pl-3 border-l-2 space-y-1">
                ...
                <div className="flex flex-col gap-1 pt-2">
                  <button 
                    onClick={() => setEditor({ type: "new-lesson", moduleId: module.id })} 
                    className="text-[10px] font-bold text-primary flex items-center gap-1 uppercase hover:underline"
                  >
                    <Plus size={12}/> Nova Aula
                  </button>
                  <button 
                    onClick={() => setEditor({ type: "quiz", moduleId: module.id, moduleTitle: module.titulo })} 
                    className="text-[10px] font-bold text-orange-500 flex items-center gap-1 uppercase hover:underline"
                  >
      ...
      {/* Footer Fixo no Fundo - Botões Lado a Lado */}
      <div className="p-4 border-t bg-muted/20">
        <div className="grid grid-cols-2 gap-2">
          <Button 
            variant="outline" 
            className="w-full gap-2 text-xs sm:text-sm px-2" 
            onClick={() => router.push("/treinamentos/visualizar")}
          >
            <LogOut size={16}/> Sair
          </Button>
          <Button 
            className="w-full bg-green-600 hover:bg-green-700 gap-2 font-bold text-xs sm:text-sm px-2" 
